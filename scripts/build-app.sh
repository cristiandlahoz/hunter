...
ICON="$ROOT/Resources/WattHound.icns"
HOUND_SVG="$ROOT/Sources/WattHound/Resources/BloodhoundMenuBar.svg"
if [[ ! -f "$ICON" || "$HOUND_SVG" -nt "$ICON" || "$ROOT/scripts/make-icon.swift" -nt "$ICON" ]]; then
  ...
  iconutil -c icns "$WORK/WattHound.iconset" -o "$ICON"
fi
cp "$ICON" "$CONTENTS/Resources/WattHound.icns"
